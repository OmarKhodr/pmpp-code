...
__global__
void vecMaxKernel(float *A, float *B, float *C, int n) {
...
void vecMaxGpu(float *A_h, float *B_h, float *C_h, int n) {
	ko::Timer gpu_timer;
	...
	vecMaxKernel<<<num_blocks, num_threads_per_block>>>(A_d, B_d, C_d, n);
